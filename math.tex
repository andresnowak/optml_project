...
$t = 1, 2, \dots$ The matrix optimizers use the Muon/DynMuon lookahead
momentum convention. In the released DynMuon reference this is stored in
sum form as
\begin{equation}
\label{eq:momentum}
...
stationary gradients. This matches the reference DynMuon exactly. Some
Muon-style implementations in this repository store the equivalent
EMA-scaled buffer $\widetilde B_t=(1-\mu)B_t$ and form
$\widetilde M_t=(1-\mu)M_t$; because Muon, RelMuon, InputMuon, and Kaon
normalize or replace the update spectrum, this positive scalar does not change
their direction. AdamW auxiliary parameters use Adam moments and should not be
described as Nesterov.
...
shared clock \eqref{eq:schedule} plus a bounded local lean:
\begin{equation}
\label{eq:router}
p_{t,l} \;=\; \clip\Bigl(p_t +
\clip\bigl(\beta\, z_{t,l},\, -\ell_{\max},\, +\ell_{\max}\bigr),\;
p_{\min},\; p_{\max}\Bigr),
\qquad
z_{t,l} = \frac{x_{t,l} - \bar{x}_t}{\tilde{\sigma}_t},
\end{equation}
where $\bar x_t$ and $\tilde\sigma_t$ are exponential moving averages (decay
$\beta_r$) of the cross-layer mean and cross-layer standard deviation of the
proxy, both computed from steps $s < t$ (\texttt{lean\_norm="zscore"};
$\tilde\sigma_t \equiv 1$ recovers the legacy raw lean). Subtracting
$\bar x_t$ removes the global temporal trend of the proxy (which the clock
already owns); dividing by $\tilde\sigma_t$ makes $\beta$ a gain in
``$p$-units per cross-layer standard deviation''. The inner clip
$\ell_{\max}$ bounds the total lean. Both guards are necessary in practice:
the stable-rank proxy is heavy-tailed (deviations exceeding $+50$ on early
layers were logged in the 20k-step run), and with a raw gain the router
saturates into a bang-bang controller whose exponents sit at the
$[p_{\min}, p_{\max}]$ boundaries rather than perturbing the schedule. At
...
boundedness and layer separation are covered by
\texttt{test\_zscore\_lean\_is\_bounded\_and\_reduces\_to\_schedule}.

Two specification details matter in practice. First, a layer with vanishing
momentum ($\|M\|_F = 0$, e.g.\ matrices whose step-1 gradients are blocked by
zero-initialized downstream projections) has no defined exponent and a zero
update for every $p$; it contributes \emph{no} sample to the cross-layer
statistics (a single NaN sample would latch the EMA permanently, which
silently pinned every layer at $p_{\max}$ in early sweep runs; regression
test \texttt{test\_zero\_first\_step\_gradient\_does\_not\_poison\_proxy\_ema}).
Second, under \texttt{compute\_mode="reference"} the per-layer exponent is
applied through the \emph{discontinuous} three-phase transform
\eqref{eq:threephase}: a lean that crosses $p = 0.25$ switches between the
raw-momentum branch ($\|D\|_F = \|M\|_F$) and the polar branch
($\|D\|_F \approx \sqrt{k}$), a norm jump of order $\sqrt{k}/\|M\|_F$. With
$\ell_{\max} = 0.25$ this can only happen while the clock itself traverses
$[0, 0.5]$ ($\approx 4\%$ of training under the default schedule); studies
that route throughout training should use the continuous paths
(\texttt{compute\_mode="ns"}) or the magnitude-decoupled variant
\eqref{eq:polarfro}.
